import json

def makeCard():
    name = input("please enter card name: ")
    description = input("please enter card description: ")
    print()
    health = input("please enter card health: ")
    attack = input("please enter card attack: ")
    print()
    bloodCost = input("please enter card blood cost: ")
    boneCost = input("please enter card bone cost: ")
    energyCost = input("please enter card energy cost: ")
    print()
    rare = input("is this card rare? [y/n]: ")
    hammer = input("can this card be hammer? [y/n]: ")
    sac = input("can this card be sacrifice? [y/n]: ")

    workingSigils = [
        "Airborne",
        "Mighty Leap",
        "Fecundity",
        "Unkillable",
        "Blue Mox",
        "Green Mox",
        "Orange Mox",
        "Great Mox",
        "Rabbit Hole",
        "Touch of Death",
        "Many Lives",
        "Trifurcated Strike",
        "Battery Bearer",
        "Repulsive",
        "Brittle",
        "Worthy Sacrifice",
        "Gem Dependant",
        "Bone King",
        "Bifurcated Strike",
        "Handy",
        "Fledgling",
        "Sprinter",
        "Squirrel Shedder",
        "Skeleton Crew",
        "Bone Digger",
        "Waterborne",
        "Ruby Heart",
        "Frozen Away",
        "Mental Gemnastics",
        "Looter",
        "Gem Animator",
        "Hefty",
        "Guardian",
        "Sharp Quills",
        "Sentry",
        "Burrower",
        "Hoarder",
        "Detonator",
        "Bomb Spewer",
        "Double Death",
        "Enlarge",
        "Disentomb",
        "Power Dice",
        "Stimulate",
        "Energy Gun",
        "True Scholar",
        "Bonehorn",
        "Boneless",
        "Attack Conduit",
        "Spawn Conduit",
        "Energy Conduit",
        "Tentacle",
        "Stinky",
        "Reconstitute",
        "Noble Sacrifice",
        "Made of Stone",
        "Side Hustle",
    ]

    flag = True
    while flag:
        sigil1 = input("please enter the first sigil: ")
        for sigil in workingSigils:
            if sigil1.lower() == sigil.lower():
                flag = False
                sigil1 = sigil
                break

        if not flag:
            break
        print("the sigil you enter is not a working sigil please try again")

    flag = True
    while flag:
        sigil2 = input("please enter the second sigil: ")
        for sigil in workingSigils:
            if sigil2.lower() == sigil.lower():
                flag = False
                sigil2 = sigil
                break

        if not flag:
            break

        print("the sigil you enter is not a working sigil please try again")

    print("Finalizing shit please wait...")

    object = {
        "name": name.capitalize(),
        "description": description.capitalize(),
        "health": 0 if health == "" else int(health),
        "attack": 0 if attack == "" else int(attack),
        "blood_cost": 0 if bloodCost == "" else int(bloodCost),
        "bone_cost": 0 if boneCost == "" else int(boneCost),
        "energy_cost": 0 if energyCost == "" else int(energyCost),
        "rare": True if rare == "y"or rare != "" else False,
        "nohammer": True if hammer == "y"or hammer != "" else False,
        "nosac": True if sac == "y" or hammer != "" else False,
        "sigils": [sigil1, sigil2],
    }

    return object

fullRule = input("do you want to generate the full ruleset or card json? [f/c]: ")
isFull = True if fullRule == "f" else False

flag = True
cards = []

while flag:
    card = makeCard()
    ans = input("do you want to make another card before exporting? [y/n]: ")

    flag = True if ans == "y" else False
    cards.append(card)

if isFull:
    ruleset = {
        "ruleset": "IMF Competitive Ruleset v0.05",
        "cards": [
            {
                "name": "Starvation",
                "sigils": ["Repulsive"],
                "attack": 1,
                "health": 1,
                "banned": True,
            },
            {
                "name": "Adder",
                "sigils": ["Touch of Death"],
                "attack": 1,
                "health": 3,
                "blood_cost": 2,
            },
            {
                "name": "Bat",
                "sigils": ["Airborne"],
                "attack": 2,
                "health": 1,
                "bone_cost": 4,
            },
            {
                "name": "Black Goat",
                "attack": 0,
                "health": 1,
                "sigils": ["Worthy Sacrifice"],
                "blood_cost": 1,
                "description": "Ported from Act 1. Act 2 sprite from Kaycee's Mod.",
            },
            {
                "name": "Bloodhound",
                "sigils": ["Guardian"],
                "attack": 2,
                "health": 3,
                "blood_cost": 2,
            },
            {
                "name": "Bullfrog",
                "sigils": ["Mighty Leap"],
                "attack": 1,
                "health": 2,
                "blood_cost": 1,
            },
            {
                "name": "Burrowing Trap",
                "sigils": ["Burrower", "Steel Trap"],
                "attack": 0,
                "health": 5,
                "blood_cost": 2,
                "banned": True,
            },
            {
                "name": "Cat",
                "sigils": ["Many Lives"],
                "attack": 0,
                "health": 1,
                "blood_cost": 1,
                "rare": True,
                "description": "Note: This card was changed to rare due to the amount of advantage given by this card.",
            },
            {
                "name": "Undead Cat",
                "attack": 3,
                "health": 6,
                "blood_cost": 1,
                "banned": True,
            },
            {
                "name": "Cockroach",
                "attack": 1,
                "health": 1,
                "sigils": ["Unkillable"],
                "bone_cost": 4,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Elk",
                "sigils": ["Sprinter"],
                "attack": 2,
                "health": 4,
                "blood_cost": 2,
            },
            {
                "name": "Elk Fawn",
                "sigils": ["Sprinter", "Fledgling"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
                "evolution": "Elk",
            },
            {
                "name": "Field Mice",
                "sigils": ["Fecundity"],
                "attack": 2,
                "health": 2,
                "blood_cost": 2,
            },
            {
                "name": "Spore Mice",
                "sigils": ["Fecundity", "Fecundity"],
                "attack": 2,
                "health": 2,
                "blood_cost": 2,
                "banned": True,
            },
            {
                "name": "Hawk",
                "sigils": ["Airborne"],
                "attack": 3,
                "health": 1,
                "blood_cost": 2,
            },
            {
                "name": "Hrokkall",
                "sigils": ["Waterborne", "Battery Bearer"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
                "rare": True,
            },
            {
                "name": "Raven Egg",
                "sigils": ["Fledgling"],
                "attack": 0,
                "health": 2,
                "evolution": "Raven",
                "blood_cost": 1,
            },
            {
                "name": "Kingfisher",
                "sigils": ["Airborne", "Waterborne"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
            },
            {
                "name": "Great Kraken",
                "sigils": ["Tentacle"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
                "rare": True,
                "description": "Note: This card's effect was changed due to it sucking.",
            },
            {
                "name": "Magpie",
                "sigils": ["Airborne", "Hoarder"],
                "attack": 1,
                "health": 1,
                "blood_cost": 2,
                "description": "This card was unused in act 2, but had art.",
            },
            {
                "name": "Mantis",
                "sigils": ["Bifurcated Strike"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
            },
            {
                "name": "Mantis God",
                "sigils": ["Trifurcated Strike"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
                "rare": True,
            },
            {
                "name": "Mole",
                "sigils": ["Burrower"],
                "attack": 0,
                "health": 4,
                "blood_cost": 1,
            },
            {
                "name": "Mole Man",
                "sigils": ["Burrower", "Mighty Leap"],
                "attack": 0,
                "health": 6,
                "blood_cost": 1,
                "rare": True,
            },
            {
                "name": "Moose Buck",
                "attack": 3,
                "health": 7,
                "sigils": ["Hefty"],
                "blood_cost": 3,
                "description": "Ported from Act 1. Act 2 sprite from Kaycee's Mod.",
            },
            {"name": "Opossum", "attack": 1, "health": 1, "bone_cost": 2},
            {
                "name": "Ouroboros",
                "sigils": ["Unkillable"],
                "attack": 1,
                "health": 1,
                "blood_cost": 2,
                "rare": True,
                "nohammer": True,
                "description": "This card gains 1 power and 1 health each time it perishes.",
            },
            {"name": "Rabbit", "attack": 0, "health": 1},
            {
                "name": "Skunk",
                "attack": 0,
                "health": 3,
                "blood_cost": 1,
                "sigils": ["Stinky"],
                "description": "Ported from Act 1. Act 2 sprite from Kaycee's Mod.",
            },
            {"name": "Coyote", "attack": 2, "health": 1, "bone_cost": 4},
            {
                "name": "Great White",
                "attack": 4,
                "health": 2,
                "blood_cost": 3,
                "sigils": ["Waterborne"],
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {"name": "Grizzly", "attack": 4, "health": 6, "blood_cost": 3},
            {
                "name": "Porcupine",
                "attack": 1,
                "health": 2,
                "sigils": ["Sharp Quills"],
                "blood_cost": 1,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Pronghorn",
                "attack": 1,
                "health": 3,
                "sigils": ["Bifurcated Strike", "Sprinter"],
                "blood_cost": 2,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Rattler",
                "attack": 3,
                "health": 1,
                "bone_cost": 6,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Raven",
                "sigils": ["Airborne"],
                "attack": 2,
                "health": 3,
                "blood_cost": 2,
            },
            {
                "name": "Salmon",
                "sigils": ["Waterborne", "Sprinter"],
                "attack": 2,
                "health": 2,
                "blood_cost": 2,
            },
            {
                "name": "River Snapper",
                "attack": 1,
                "health": 6,
                "blood_cost": 2,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Sparrow",
                "attack": 1,
                "health": 2,
                "sigils": ["Airborne"],
                "blood_cost": 1,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Strange Larva",
                "sigils": ["Fledgling"],
                "attack": 0,
                "health": 3,
                "blood_cost": 1,
                "evolution": "Strange Pupa",
                "rare": True,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Strange Pupa",
                "sigils": ["Fledgling"],
                "attack": 0,
                "health": 3,
                "blood_cost": 1,
                "banned": True,
                "evolution": "Mothman",
                "rare": True,
            },
            {
                "name": "Mothman",
                "sigils": ["Airborne"],
                "attack": 7,
                "health": 3,
                "blood_cost": 1,
                "banned": True,
                "rare": True,
            },
            {
                "name": "Turkey Vulture",
                "attack": 3,
                "health": 3,
                "sigils": ["Airborne"],
                "bone_cost": 8,
                "description": "Ported from Act 1. Act 2 sprite by syntaxevasion.",
            },
            {
                "name": "Bell Tentacle",
                "sigils": ["Tentacle"],
                "attack": 0,
                "health": 3,
                "blood_cost": 2,
                "banned": True,
            },
            {
                "name": "Hand Tentacle",
                "sigils": ["Tentacle"],
                "attack": 0,
                "health": 1,
                "blood_cost": 1,
                "banned": True,
            },
            {
                "name": "Mirror Tentacle",
                "sigils": ["Tentacle"],
                "attack": 0,
                "health": 3,
                "blood_cost": 1,
                "atkspecial": 1,
                "banned": True,
            },
            {"name": "Squirrel", "attack": 0, "health": 1, "banned": True},
            {
                "name": "Squirrel Ball",
                "sigils": ["Squirrel Shedder"],
                "attack": 0,
                "health": 1,
                "blood_cost": 1,
            },
            {
                "name": "Stoat",
                "attack": 1,
                "health": 3,
                "blood_cost": 1,
                "description": "Note: Health was increased to 3 to make it better than Bullfrog in at least one way. Bears a unique ability from Act 1.",
            },
            {
                "name": "Urayuli",
                "attack": 7,
                "health": 7,
                "blood_cost": 4,
                "rare": True,
            },
            {
                "name": "Warren",
                "sigils": ["Rabbit Hole"],
                "attack": 0,
                "health": 2,
                "blood_cost": 1,
            },
            {"name": "Wolf", "attack": 3, "health": 2, "blood_cost": 2},
            {
                "name": "Wolf Cub",
                "sigils": ["Fledgling"],
                "attack": 1,
                "health": 1,
                "blood_cost": 1,
                "evolution": "Wolf",
            },
            {
                "name": "Fox",
                "sigils": ["Sprinter", "Side Hustle"],
                "attack": 1,
                "health": 2,
                "blood_cost": 2,
                "description": "Custom card by SpelunkyGuy101.\nCard and sigil art by syntaxevasion",
            },
            {
                "name": "Curve Hopper",
                "attack": 2,
                "health": 3,
                "energy_cost": 4,
                "rare": True,
            },
            {
                "name": "Automaton",
                "attack": 1,
                "health": 1,
                "energy_cost": 2,
                "description": "Note: The energy cost of this card was reduced to make it less terrible.",
            },
            {
                "name": "Energy Bot",
                "sigils": ["Battery Bearer"],
                "attack": 0,
                "health": 1,
                "energy_cost": 2,
            },
            {
                "name": "Bolthound",
                "sigils": ["Guardian"],
                "attack": 2,
                "health": 3,
                "energy_cost": 6,
                "description": 'Note: Health was increased to 3 to make it equivalent to other "Hound" cards.',
            },
            {
                "name": "Buff Conduit",
                "sigils": ["Attack Conduit"],
                "attack": 0,
                "health": 2,
                "energy_cost": 2,
                "conduit": True,
            },
            {
                "name": "Double Gunner",
                "sigils": ["Bifurcated Strike"],
                "attack": 2,
                "health": 1,
                "energy_cost": 6,
            },
            {
                "name": "Energy Conduit",
                "sigils": ["Energy Conduit"],
                "attack": 1,
                "health": 2,
                "energy_cost": 3,
                "conduit": True,
                "rare": True,
                "description": "Note: Effect was changed to prevent infinitely buffing stim mage.",
            },
            {
                "name": "Explode Bot",
                "sigils": ["Detonator"],
                "attack": 1,
                "health": 1,
                "energy_cost": 2,
            },
            {
                "name": "Factory Conduit",
                "sigils": ["Spawn Conduit"],
                "attack": 0,
                "health": 2,
                "energy_cost": 3,
                "conduit": True,
            },
            {
                "name": "Gamblobot",
                "sigils": ["Power Dice"],
                "attack": 0,
                "health": 1,
                "energy_cost": 3,
                "active": True,
                "description": "Note: The cost of this ability was increased to 2 energy due to this card being very strong for its cost.",
            },
            {
                "name": "Insectodrone",
                "sigils": ["Airborne"],
                "attack": 1,
                "health": 1,
                "energy_cost": 3,
            },
            {
                "name": "L33pB0t",
                "sigils": ["Mighty Leap"],
                "attack": 0,
                "health": 2,
                "energy_cost": 1,
            },
            {
                "name": "M3atB0t",
                "sigils": ["Noble Sacrifice"],
                "attack": 0,
                "health": 1,
                "energy_cost": 2,
            },
            {
                "name": "49er",
                "sigils": ["Sprinter"],
                "attack": 1,
                "health": 1,
                "energy_cost": 2,
            },
            {
                "name": "Null Conduit",
                "attack": 0,
                "health": 1,
                "energy_cost": 1,
                "conduit": True,
            },
            {
                "name": "Plasma Jimmy",
                "sigils": ["Energy Gun"],
                "attack": 0,
                "health": 3,
                "energy_cost": 2,
                "rare": True,
                "active": True,
            },
            {
                "name": "Steel Mice",
                "sigils": ["Fecundity"],
                "attack": 1,
                "health": 1,
                "energy_cost": 4,
                "description": "Note: Fecundity is nerfed as per Kaycee's Mod",
            },
            {
                "name": "Sentry Drone",
                "sigils": ["Sentry"],
                "attack": 0,
                "health": 1,
                "energy_cost": 1,
                "description": "Note: The sentry sigil now triggers when this card is pushed by another card.",
            },
            {
                "name": "Sentry Spore",
                "sigils": ["Sentry", "Sentry"],
                "attack": 0,
                "health": 1,
                "energy_cost": 1,
                "banned": True,
            },
            {
                "name": "Mrs. Bomb",
                "sigils": ["Bomb Spewer"],
                "attack": 1,
                "health": 2,
                "energy_cost": 3,
                "rare": True,
            },
            {
                "name": "Shutterbug",
                "sigils": ["Touch of Death", "Sentry"],
                "attack": 1,
                "health": 1,
                "energy_cost": 5,
                "rare": True,
                "description": "Note: The sentry sigil now triggers when this card is pushed by another card.",
            },
            {
                "name": "Sniper Bot",
                "attack": 1,
                "health": 1,
                "energy_cost": 3,
                "banned": True,
            },
            {
                "name": "Steambot",
                "attack": 3,
                "health": 2,
                "energy_cost": 6,
                "description": "Note: This card's attack was increased to make it worth 6 energy.",
            },
            {
                "name": "Mox Module",
                "sigils": ["Great Mox"],
                "attack": 0,
                "health": 3,
                "energy_cost": 3,
                "rare": True,
            },
            {
                "name": "Thick Droid",
                "attack": 1,
                "health": 4,
                "energy_cost": 5,
                "description": "Note: This card's health was increased to make it worth 5 energy.",
            },
            {
                "name": "Draugr",
                "sigils": ["Frozen Away"],
                "attack": 0,
                "health": 1,
                "bone_cost": 1,
                "nosac": True,
            },
            {
                "name": "Banshee",
                "sigils": ["Airborne"],
                "attack": 1,
                "health": 1,
                "bone_cost": 2,
            },
            {
                "name": "Bonehound",
                "sigils": ["Guardian"],
                "attack": 2,
                "health": 3,
                "bone_cost": 7,
            },
            {
                "name": "Bone Lord's Horn",
                "sigils": ["Bonehorn"],
                "attack": 1,
                "health": 1,
                "bone_cost": 3,
                "rare": True,
                "active": True,
                "description": "Note: This card gives only 1 bone per 1 energy due to the previous effect being overpowered in combination with Bone Heap.",
            },
            {
                "name": "Bone Heap",
                "sigils": ["Enlarge"],
                "attack": 0,
                "health": 1,
                "rare": True,
                "active": True,
            },
            {
                "name": "Broken Obol 1",
                "sigils": ["Sharp Quills"],
                "attack": 0,
                "health": 2,
                "bone_cost": 3,
            },
            {
                "name": "Broken Obol 2",
                "sigils": ["Bone King"],
                "attack": 0,
                "health": 2,
                "bone_cost": 3,
            },
            {
                "name": "Dead Hand",
                "sigils": ["Handy"],
                "attack": 1,
                "health": 1,
                "bone_cost": 5,
                "description": "Note: Was made common due to lack of use.",
            },
            {
                "name": "Pharaoh's Pets",
                "sigils": ["Worthy Sacrifice", "Many Lives"],
                "attack": 0,
                "health": 1,
                "bone_cost": 6,
                "rare": True,
                "description": "Note: This card was made rare due to it's strength. This card will become a cat after its worthy sacrifice sigil is used.",
            },
            {
                "name": "Drowned Soul",
                "sigils": ["Waterborne", "Touch of Death"],
                "attack": 1,
                "health": 1,
                "bone_cost": 4,
                "rare": True,
            },
            {
                "name": "The Walkers",
                "sigils": ["Bone King"],
                "attack": 1,
                "health": 2,
                "bone_cost": 4,
            },
            {"name": "Frank & Stein", "attack": 2, "health": 2, "bone_cost": 5},
            {
                "name": "Ghost Ship",
                "sigils": ["Waterborne", "Skeleton Crew"],
                "attack": 0,
                "health": 1,
                "bone_cost": 4,
            },
            {
                "name": "Gravedigger",
                "sigils": ["Bone Digger"],
                "attack": 0,
                "health": 3,
                "bone_cost": 1,
            },
            {
                "name": "Sporedigger",
                "sigils": ["Bone Digger", "Bone Digger"],
                "attack": 0,
                "health": 3,
                "bone_cost": 1,
                "banned": True,
            },
            {
                "name": "Headless Horseman",
                "sigils": ["Airborne", "Sprinter"],
                "attack": 5,
                "health": 5,
                "bone_cost": 13,
                "rare": True,
            },
            {"name": "Mummy Lord", "attack": 3, "health": 3, "bone_cost": 8},
            {
                "name": "Necromancer",
                "sigils": ["Double Death"],
                "attack": 1,
                "health": 2,
                "bone_cost": 3,
                "rare": True,
                "description": "This was the most annoying card to program.",
            },
            {
                "name": "Revenant",
                "sigils": ["Brittle"],
                "attack": 3,
                "health": 1,
                "bone_cost": 3,
            },
            {
                "name": "Sarcophagus",
                "sigils": ["Fledgling"],
                "attack": 0,
                "health": 2,
                "bone_cost": 4,
                "evolution": "Mummy Lord",
            },
            {
                "name": "Skeleton",
                "sigils": ["Brittle"],
                "attack": 1,
                "health": 1,
                "banned": True,
            },
            {
                "name": "Skelemagus",
                "sigils": ["Brittle", "Gem Dependant"],
                "attack": 4,
                "health": 1,
                "energy_cost": 4,
                "description": "Note: Was given energy cost due to being too spammable and easy to draw into with Blue Mage.",
            },
            {
                "name": "Tomb Robber",
                "sigils": ["Disentomb"],
                "attack": 0,
                "health": 2,
                "rare": True,
                "active": True,
                "description": "Note: Withered corpses have the Boneless sigil and will not drop bones.This change was made to prevent multiple infinite combos involving Tomb Robber.",
            },
            {
                "name": "Zombie",
                "attack": 1,
                "health": 1,
                "bone_cost": 3,
                "sigils": ["Stinky"],
                "description": "This card was given the stinky sigil to differentiate it from Opossom. Suggested by SpelunkyGuy101.",
            },
            {
                "name": "Floating Eye",
                "attack": 1,
                "health": 3,
                "bone_cost": 5,
                "sigils": ["Airborne", "Sentry"],
                "description": "Custom card by SYNTHIA_2000",
            },
            {
                "name": "Gargoyle",
                "attack": 1,
                "health": 4,
                "bone_cost": 6,
                "sigils": ["Airborne", "Made of Stone"],
                "description": "Custom card by syntaxevasion.\nLong overdue.",
                "nosac": True,
            },
            {
                "name": "Gourmage",
                "sigils": ["Enlarge"],
                "attack": 0,
                "health": 2,
                "mox_cost": ["Green"],
                "active": True,
            },
            {
                "name": "Blue Mage",
                "sigils": ["Mental Gemnastics"],
                "attack": 0,
                "health": 1,
                "mox_cost": ["Blue"],
                "nohammer": True,
                "description": "Note: Made unhammerable to allow for better mox stuff in future",
            },
            {
                "name": "Blue Sporemage",
                "sigils": ["Mental Gemnastics", "Mental Gemnastics"],
                "attack": 0,
                "health": 1,
                "mox_cost": ["Blue"],
                "banned": True,
            },
            {
                "name": "Hover Mage",
                "sigils": ["Airborne"],
                "attack": 1,
                "health": 1,
                "mox_cost": ["Blue"],
            },
            {
                "name": "Force Mage",
                "sigils": ["Repulsive"],
                "attack": 0,
                "health": 1,
                "mox_cost": ["Blue"],
                "description": "Dies to removal.",
            },
            {
                "name": "Gem Fiend",
                "sigils": ["Gem Dependant"],
                "attack": 2,
                "health": 1,
                "mox_cost": ["Blue"],
            },
            {
                "name": "Green Mage",
                "attack": 0,
                "health": 2,
                "mox_cost": ["Green"],
                "atkspecial": 0,
                "description": 'This card\'s attack is equal to the number of "Mox" cards you possess.',
            },
            {"name": "Junior Sage", "attack": 1, "health": 2, "mox_cost": ["Green"]},
            {
                "name": "Mage Knight",
                "sigils": ["Gem Dependant"],
                "attack": 1,
                "health": 3,
                "mox_cost": ["Orange"],
            },
            {
                "name": "Master Bleene",
                "sigils": ["True Scholar"],
                "attack": 0,
                "health": 4,
                "mox_cost": ["Green", "Blue"],
                "rare": True,
                "active": True,
            },
            {
                "name": "Master Goranj",
                "sigils": ["Gem Dependant", "Bifurcated Strike"],
                "attack": 2,
                "health": 4,
                "mox_cost": ["Green", "Orange"],
                "rare": True,
                "description": "Note: This card was given Bifurcated Strike due to being considerably weaker than the other masters, and to give it the ability to defend adjacent gems (potentially). Suggested by SpelunkyGuy101.",
            },
            {
                "name": "Master Orlu",
                "sigils": ["Airborne", "Looter"],
                "attack": 2,
                "health": 3,
                "mox_cost": ["Orange", "Blue"],
                "rare": True,
                "description": "Note: Attack was increased to make it an acceptable rare card and to demonstrate its sigil effect more clearly.",
            },
            {
                "name": "Bleene's Mox",
                "sigils": ["Blue Mox", "Green Mox"],
                "attack": 0,
                "health": 2,
                "rare": True,
                "nosac": True,
            },
            {
                "name": "Goranj's Mox",
                "sigils": ["Green Mox", "Orange Mox"],
                "attack": 0,
                "health": 2,
                "rare": True,
                "nosac": True,
            },
            {
                "name": "Orlu's Mox",
                "sigils": ["Orange Mox", "Blue Mox"],
                "attack": 0,
                "health": 2,
                "rare": True,
                "nosac": True,
            },
            {
                "name": "Emerald Mox",
                "sigils": ["Green Mox"],
                "attack": 0,
                "health": 1,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Ruby Mox",
                "sigils": ["Orange Mox"],
                "attack": 0,
                "health": 1,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Sapphire Mox",
                "sigils": ["Blue Mox"],
                "attack": 0,
                "health": 1,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Magnus Mox",
                "sigils": ["Great Mox"],
                "attack": 0,
                "health": 2,
                "nosac": True,
                "banned": True,
                "rare": True,
            },
            {
                "name": "Muscle Mage",
                "sigils": ["Hefty"],
                "attack": 1,
                "health": 1,
                "mox_cost": ["Green"],
            },
            {
                "name": "Orange Mage",
                "sigils": ["Gem Animator"],
                "attack": 0,
                "health": 1,
                "mox_cost": ["Orange"],
            },
            {
                "name": "Practice Wizard",
                "attack": 0,
                "health": 3,
                "sigils": ["Made of Stone"],
                "mox_cost": ["Orange"],
            },
            {
                "name": "Mage Pupil",
                "sigils": ["Gem Dependant"],
                "attack": 1,
                "health": 1,
            },
            {
                "name": "Ruby Golem",
                "sigils": ["Ruby Heart"],
                "attack": 1,
                "health": 1,
                "mox_cost": ["Orange"],
                "nosac": True,
            },
            {
                "name": "Stim Mage",
                "sigils": ["Stimulate"],
                "attack": 0,
                "health": 2,
                "mox_cost": ["Green"],
                "active": True,
            },
            {
                "name": "Mox Tower",
                "sigils": ["Sentry", "Orange Mox"],
                "attack": 0,
                "health": 2,
                "mox_cost": ["Orange"],
                "description": "Custom card by SpelunkyGuy101",
            },
            {
                "name": "Mox Bunker",
                "sigils": ["Sharp Quills", "Blue Mox"],
                "attack": 0,
                "health": 2,
                "mox_cost": ["Blue"],
                "description": "Custom Card by BishopAdams",
            },
            {
                "name": "Mox Encampment",
                "sigils": ["Stinky", "Green Mox"],
                "attack": 0,
                "health": 2,
                "mox_cost": ["Green"],
                "description": "Custom Card by BishopAdams",
            },
            {
                "name": "Thief Mage",
                "attack": 1,
                "health": 1,
                "sigils": ["Looter"],
                "mox_cost": ["Blue", "Orange"],
                "description": "Custom card by SpelunkyGuy101",
                "banned": "for being too op",
            },
            {
                "name": "Life Mage",
                "attack": 0,
                "health": 2,
                "sigils": ["Unkillable"],
                "mox_cost": ["Blue"],
                "bone_cost": 3,
                "description": "Custom card by SpelunkyGuy101",
            },
            {
                "name": "Dragon Egg",
                "attack": 0,
                "health": 2,
                "sigils": ["Fledgling"],
                "mox_cost": ["Orange"],
                "evolution": "Ruby Dragon",
                "description": "Custom card by SpelunkyGuy101.\nUpdated art by syntaxevasion",
            },
            {
                "name": "Ruby Dragon",
                "attack": 2,
                "health": 2,
                "sigils": ["Bifurcated Strike"],
                "mox_cost": ["Orange"],
                "banned": True,
            },
            {
                "name": "Shower Mage",
                "mox_cost": ["Green"],
                "sigils": ["Stinky"],
                "attack": 1,
                "health": 1,
                "description": "Custom card by SpelunkyGuy101",
            },
            {
                "name": "Withered Corpse",
                "sigils": ["Brittle", "Boneless"],
                "attack": 1,
                "health": 1,
                "banned": True,
            },
            {
                "name": "Empty Vessel",
                "attack": 0,
                "health": 3,
                "energy_cost": 1,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Leaping Vessel",
                "sigils": ["Mighty Leap"],
                "attack": 0,
                "health": 2,
                "energy_cost": 2,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Energized Vessel",
                "sigils": ["Battery Bearer"],
                "attack": 0,
                "health": 2,
                "energy_cost": 3,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Bloody Vessel",
                "sigils": ["Noble Sacrifice"],
                "attack": 0,
                "health": 2,
                "energy_cost": 4,
                "banned": True,
                "description": "Note: Energy cost increased due to opponent not having enough time to build a board before bloody vessel is usable.",
            },
            {
                "name": "Splintered Vessel",
                "sigils": ["Sharp Quills"],
                "attack": 0,
                "health": 2,
                "energy_cost": 2,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Conductive Vessel",
                "attack": 0,
                "health": 2,
                "energy_cost": 2,
                "nosac": True,
                "conduit": True,
                "banned": True,
            },
            {
                "name": "Contaminated Vessel",
                "sigils": ["Stinky"],
                "attack": 0,
                "health": 2,
                "energy_cost": 2,
                "nosac": True,
                "banned": True,
            },
            {
                "name": "Geck",
                "attack": 1,
                "health": 1,
                "rare": True,
                "banned": True,
                "description": "Ported from Act 1. Act 2 sprite from Kaycee's Mod.",
            },
            {
                "name": "Acid Squirrel",
                "attack": 0,
                "health": 1,
                "sigils": ["Reconstitute"],
                "banned": True,
                "description": "Card and Reconstitute sigil art by Sebas534.\nThe most broken card, hopefully no longer.",
            },
            {
                "name": "Shambling Cairn",
                "attack": 0,
                "health": 1,
                "bone_cost": -1,
                "sigils": ["Sprinter"],
                "nohammer": True,
                "banned": True,
                "description": "Custom card by Garf, zpan0317, and Me!",
            },
            {
                "name": "Moon Shard",
                "sigils": ["Mighty Leap", "Made of Stone"],
                "attack": 0,
                "health": 5,
                "energy_cost": 6,
                "nosac": False,
                "description": "Emits a pale white light. You feel as though it calls to its brethren.",
                "banned": True,
            },
            {"name": "The Moon", "attack": 1, "health": 40, "banned": True},
        ],
        "sigils": {
            "Airborne": "A card bearing this sigil will strike an opponent directly, even if there is a creature opposing it.",
            "Attack Conduit": "Other creatures within a circuit completed by a card bearing this sigil gain 1 power.",
            "Battery Bearer": "When a card bearing this sigil is played, it provides an energy cell to its owner.",
            "Bifurcated Strike": "A card bearing this sigil will strike each opposing space to the left and right of the space across from it.",
            "Blue Mox": "While a card bearing this sigil is on the board, it provides a blue gem to its owner.",
            "Bomb Spewer": "When a card bearing this sigil is played, fill all empty spaces with explode bots.",
            "Bone Digger": "At the end of the owner's turn, a card bearing this sigil will generate 1 bone.",
            "Bone King": "When a card bearing this sigil dies, 4 bones are awarded instead of 1.",
            "Bonehorn": "Pay 1 energy to gain 1 bone.",
            "Boneless": "When a card bearing this sigil dies, no bones are awarded.",
            "Brittle": "After attacking, a card bearing this sigil perishes.",
            "Burrower": "When an empty space would be struck, a card bearing this sigil will move to that space to receive the strike instead.",
            "Detonator": "When a card bearing this sigil dies, the creature opposing it, as well as adjacent friendly creatures, are dealt 10 damage.",
            "Disentomb": "Pay 1 bone to create a withered corpse in your hand.",
            "Double Death": "When another creature you own dies, it is returned to life and dies again immediately.",
            "Energy Conduit": "If a card bearing this sigil is part of a completed circuit, your maximum energy increases by 2.",
            "Energy Gun": "Pay 1 energy to deal 1 damage to the creature across from a card bearing this sigil.",
            "Enlarge": "Pay 2 bones to increase the power and health of a card bearing this sigil by 1.",
            "Fecundity": "When a card bearing this sigil is played, a copy of it is created in your hand.",
            "Fledgling": "A card bearing this sigil will grow into a more powerful form after 1 turn on the board.",
            "Frozen Away": "When a card bearing this sigil perishes, the creature inside is released in its place.",
            "Gem Animator": "Mox cards on the owner's side of the board gain 1 power.",
            "Gem Dependant": "If a card bearing this sigil's owner controls no mox cards, a card bearing this sigil perishes.",
            "Great Mox": "While a card bearing this sigil is on the board, it provides a green, orange, and blue gem to its owner.",
            "Green Mox": "While a card bearing this sigil is on the board, it provides a green gem to its owner.",
            "Guardian": "When an opposing creature is placed opposite to an empty space, a card bearing this sigil will move to that empty space.",
            "Handy": "When a card bearing this sigil is played, discard your hand then draw a new hand of 4 cards.",
            "Hefty": "At the end of the owner's turn, a card bearing this sigil will move in the direction inscribed in the sigil. creatures in the way will be pushed in the same direction.",
            "Hoarder": "When a card bearing this sigil is played, you may search your deck for any card and take it into your hand.",
            "Looter": "When a card bearing this sigil deals damage directly, draw a card for each damage dealt.",
            "Made of Stone": "A card bearing this sigil is immune to the effects of touch of death and stinky.",
            "Many Lives": "When a card bearing this sigil is sacrificed it does not perish.",
            "Marrow Sucker": "Pay 2 bones to heal a card bearing this sigil.",
            "Mental Gemnastics": "When a card bearing this sigil is played, you draw cards equal to the amount of mox cards on your side of the board.",
            "Mighty Leap": "A card bearing this sigil will block an opposing creature bearing the airborne sigil.",
            "Orange Mox": "While a card bearing this sigil is on the board, it provides an orange gem to its owner.",
            "Power Dice": "Pay 2 energy to set the power of a card bearing this sigil randomly between 1 and 6.",
            "Rabbit Hole": "When a card bearing this sigil is played, a Rabbit is created in your hand.",
            "Reconstitute": "A card bearing this sigil returns to your hand 2 turns after it persihes.",
            "Repulsive": "If a creature would attack a card bearing this sigil, it does not.",
            "Ruby Heart": "When a card bearing this sigil perishes, a ruby mox is created in its place.",
            "Noble Sacrifice": "A card bearing this sigil is counted as 2 blood rather than 1 blood when sacrificed.",
            "Scopophobic": "A card bearing this sigil gains 1 power when not facing an enemy creature.",
            "Sentry": "When a creature moves into the space opposing a card bearing this sigil, they are dealt 1 damage.",
            "Sharp Quills": "Once a card bearing this sigil is struck, the striker is then dealt a single damage point.",
            "Side Hustle": "When a card bearing this sigil deals damage directly, draw a card from your side deck for each damage dealt.",
            "Skeleton Crew": "At the end of the owner's turn, a card bearing this sigil will move in the direction inscribed in the sigil and drop a skeleton in its old space.",
            "Spawn Conduit": "Empty spaces within a circuit completed by a card bearing this sigil spawn L33pB0ts at the end of the owner's turn.",
            "Sprinter": "At the end of the owner's turn, a card bearing this sigil will move in the direction inscribed in the sigil.",
            "Squirrel Shedder": "At the end of the owner's turn, a card bearing this sigil will move in the direction inscribed in the sigil and drop a squirrel in their old space.",
            "Stinky": "The creature opposing a card bearing this sigil loses 1 Power.",
            "Stimulate": "Pay 4 energy to increase the power and health of a card bearing this sigil by 1.",
            "Tentacle": "Includes effect of Waterborne. In addition, a card bearing this sigil will transform into a random tentacle at the start of each turn.",
            "Touch of Death": "When a card bearing this sigil damages another creature, that creature perishes.",
            "Trifurcated Strike": "A card bearing this sigil will strike each opposing space to the left, right, and center of it.",
            "True Scholar": "If you have a blue gem, sacrifice a card bearing this sigil to draw 3 cards.",
            "Unkillable": "When a card bearing this sigil perishes, a copy of it is created in your hand.",
            "Waterborne": "A card bearing this sigil submerges itself during its opponent's turn. while submerged, opposing creatures attack its owner directly.",
            "Worthy Sacrifice": "A card bearing this sigil is counted as 3 blood rather than 1 blood when sacrificed.",
        },
        "working_sigils": [
            "Airborne",
            "Mighty Leap",
            "Fecundity",
            "Unkillable",
            "Blue Mox",
            "Green Mox",
            "Orange Mox",
            "Great Mox",
            "Rabbit Hole",
            "Touch of Death",
            "Many Lives",
            "Trifurcated Strike",
            "Battery Bearer",
            "Repulsive",
            "Brittle",
            "Worthy Sacrifice",
            "Gem Dependant",
            "Bone King",
            "Bifurcated Strike",
            "Handy",
            "Fledgling",
            "Sprinter",
            "Squirrel Shedder",
            "Skeleton Crew",
            "Bone Digger",
            "Waterborne",
            "Ruby Heart",
            "Frozen Away",
            "Mental Gemnastics",
            "Looter",
            "Gem Animator",
            "Hefty",
            "Guardian",
            "Sharp Quills",
            "Sentry",
            "Burrower",
            "Hoarder",
            "Detonator",
            "Bomb Spewer",
            "Double Death",
            "Enlarge",
            "Disentomb",
            "Power Dice",
            "Stimulate",
            "Energy Gun",
            "True Scholar",
            "Bonehorn",
            "Boneless",
            "Attack Conduit",
            "Spawn Conduit",
            "Energy Conduit",
            "Tentacle",
            "Stinky",
            "Reconstitute",
            "Noble Sacrifice",
            "Made of Stone",
            "Side Hustle",
        ],
    }

    ruleName = input("Enter the name of the ruleset you want to create: ")
    print("adding card to ruleset please wait...")
    for card in cards:
        print(f"card {card['name']} added")
        ruleset["cards"].append(card)
    
    ruleset["ruleset"] = ruleName

    print("exporting ruleset please wait...")

    file = open("ruleset.json", "w")
    file.write(json.dumps(ruleset, indent=4))

    print("export complete. the file(ruleset.json) has been created in same directory as this script.")

else:
    print("here all the card you created:")
    for card in cards:
        print("====================\n")
        print(json.dumps(card))
        print("\n====================")

